...
class API(object):
    def __init__(self, checker, period):
    ...
    def returnTicker(self):
        ...
        for pair in self.pairs:
            period_str = self.period_dict[self.period]
            subStart = (time.time() - 300) * 1000
            subEnd = (time.time()) * 1000
            url = f'https://api.binance.com/api/v1/klines?symbol={pair}&interval={period_str}&startTime={int(subStart)}&endTime={int(subEnd)}&limit=1000'
            for k in range(10):
                try:
            ...
            columns.extend([pair + '_open', pair + '_high', pair + '_low', pair + '_close', pair + '_volume'])
    ...
    def returnChartData(self, pair, start, end, period):
        ...
        columns = [pair + '_open', pair + '_high', pair + '_low', pair + '_close', pair + '_volume']
